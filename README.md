...
## Route to Live
Deployment is driven by pull request labels on top of the standard PR Verify pipeline (build/test run automatically for all non-draft PRs; a Terraform plan against Development runs on open/sync/reopen/ready_for_review when `deploy-dev` is not applied):
