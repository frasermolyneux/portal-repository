resource "azurerm_linux_web_app" "app_v2" {
  ...
  app_settings = {
    ...
    "AzureAppConfiguration__ManagedIdentityClientId" = local.repository_webapi_identity_client_id
    ...
    "sql_connection_string"         = format("Server=tcp:%s;Authentication=Active Directory Default; Database=%s;User ID=%s;", data.azurerm_mssql_server.core.fully_qualified_domain_name, local.sql_database_name, local.repository_webapi_identity_client_id)
