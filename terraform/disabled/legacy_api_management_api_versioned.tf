...
locals {
  ...
  legacy_backend_mapping = {
    "v1" = {
      ...
      hostname     = azurerm_linux_web_app.legacy_app_v1.default_hostname
      protocol     = "http"
      tls_validate = true
    ...
    "v2" = {
      ...
      hostname     = azurerm_linux_web_app.legacy_app_v2.default_hostname
      protocol     = "http"
      tls_validate = true
...
resource "azurerm_api_management_api" "legacy_repository_api_versioned" {
  for_each = toset(local.legacy_versioned_apis)
...
resource "azurerm_api_management_api_policy" "legacy_repository_api_policy_versioned" {
  for_each = azurerm_api_management_api.legacy_repository_api_versioned
...
<policies>
  ...
  ? azurerm_api_management_backend.legacy_webapi_api_management_backend_versioned[local.legacy_get_major_version[each.key]].name
  : azurerm_api_management_backend.legacy_webapi_api_management_backend_versioned[local.legacy_default_backend_version].name
}" />
