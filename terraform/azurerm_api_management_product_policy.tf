...
<policies>
  <inbound>
      ...
      <authentication-managed-identity resource="${local.repository_api.application.application.client_id}"${format(" client-id=\"%s\"", local.api_management_identity.client_id)} />
  </inbound>
  <backend>
      <forward-request />
  </backend>
