...
<policies>
  <inbound>
      ...
      <authentication-managed-identity resource="${local.repository_api.application.primary_identifier_uri}"${format(" client-id=\"%s\"", local.api_management_identity.client_id)} />
  </inbound>
  <backend>
      <forward-request />
  </backend>
